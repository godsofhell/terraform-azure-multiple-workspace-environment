...
resource "azurerm_subnet" "network_subnets" {
  ...
  //telling terraform that subnet needs to be created after virtual network is created
  depends_on = [ azurerm_virtual_network.virtual_network ]
}

resource "azurerm_network_interface" "network_interfaces" {
  // Create a map for easy lookup of network interface details
  //this for_Each will loop through each network interface defined in the variable
